...
*Options Princing in R*
https://github.com/rmcd1024/derivmkts

*Query data from the U.S. Securities and Exchange Commission*
...
The cumulative returns of Apple during this period is `r round(100*sum(StockReturns[,2]),1)`%.
<br>
The average daily returns of Apple during this period is `r round(100*mean(StockReturns[,2]),3)`%;
<br>
...
<br>
*Change to:*
*mytickers = c("SPY", "AAPL", "YHOO") and perform the same actions as in Q2*
<br>
<hr>
<hr>
...
nrow(transposedData) which gives the answer `r nrow(transposedData)`
ncol(transposedData) which gives the answer `r ncol(transposedData)`
<br>
Question2
<br>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
...
1. We want to re-do all this analysis with data since 2002-01-01: what change do we need to make in the code (hint: all you need to change is one line - exactly 1 number! - in data.R file), and how can you get the new exercise set with the data since 2002-01-01?
...
Question 1
<br>
*change startDate = "2002-01-01"* and run the file again
<br>
<br>
Question 2
<br>
Add new shares to the line mytickers = c("SPY, "AAPL", "YHOO") and run the report
<br>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio = apply(StockReturns,1,mean)
names(portfolio) <- rownames(StockReturns)
...
Question 1
<br>
*Returns the number 20 or 60 if we change from ";" to ","*
<br>
Question 2
<br>
```{r eval = FALSE, echo=TRUE}
...
```
<br>
*Error, because we add a non-numeric argument to binary operator*
Mydata consists of 20 row but we add a matrix with 40 rows
<br>
